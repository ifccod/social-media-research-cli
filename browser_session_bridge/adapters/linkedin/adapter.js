...
import {
  LINKEDIN_TAB_PATTERNS,
  isLinkedInTabUrl,
  sameLinkedInRequestContext,
  validLinkedInRequest
} from "./contract.js";
import { invokeLinkedInPageRuntime } from "./page_runtime.js";

const OPERATION_ID = /^[A-Za-z0-9_-]{1,96}$/;
let operationSequence = 0;

function nextOperationID(requestID) {
  if (typeof requestID === "string" && OPERATION_ID.test(requestID)) return requestID;
  operationSequence = (operationSequence + 1) % Number.MAX_SAFE_INTEGER;
  return `linkedin_${Date.now().toString(36)}_${operationSequence.toString(36)}`;
}
...
export class LinkedInSessionAdapter {
  ...
  async routeRequest(message, context = {}) {
    if (!validLinkedInRequest(message)) return { error: "invalid_request" };
    ...
    try {
      ...
      if (!sameLinkedInRequestContext(tab.url || "", message.referer)) {
        if (/linkedin\.com\/(?:authwall|login|signup)/i.test(tab.url || "")) {
      ...
      cancelPage = () => cancellation || (cancellation = this.cancelInPage(tab.id, operationID));
      ...
      const result = await this.runInPage(tab.id, {
        kind: "request", operation_id: operationID, path: message.path, entries: message.entries
      });
      throwIfAborted(signal);
      if (result.ok !== true) {
      ...
      return { error: code === "request_aborted" ? "request_failed" : "runtime_unavailable" };
    } finally {
      removeAbort();
      removeCancel();
      if (signal?.aborted && cancelPage) await cancelPage().catch(() => undefined);
    }
  }
  ...
  async routeSession(context = {}) {
    try {
      ...
      if (result.ok !== true) return { error: result.error || "request_failed" };
      const payload = this.sessionPayload(result.payload);
      return payload ? { payload } : { error: "invalid_response" };
    } catch (error) {
      return { error: String(error?.message || "") === "tab_unavailable" ? "tab_unavailable" : "runtime_unavailable" };
    }
  }

  async linkedinTabs() {
    return this.chrome.tabs.query({ url: LINKEDIN_TAB_PATTERNS });
  }

  async pickTab(expectedURL = null, signal = null) {
    throwIfAborted(signal);
    const tabs = await this.linkedinTabs();
    const tab = expectedURL
      ? tabs.find((item) => sameLinkedInRequestContext(item.url || "", expectedURL))
  ...
  async runInPage(tabId, input) {
    ...
    if (!tab || !isLinkedInTabUrl(tab.url || "")) throw new Error("tab_unavailable");
    if (tab.status && tab.status !== "complete") throw new Error("runtime_unavailable");
    const results = await this.chrome.scripting.executeScript({
      target: { tabId }, world: "MAIN", injectImmediately: true,
      func: invokeLinkedInPageRuntime, args: [input]
    });
    const result = Array.isArray(results) && results.length === 1 ? results[0]?.result : null;
    if (!isRecord(result) || typeof result.ok !== "boolean") throw new Error("invalid_response");
    return result;
  }

  async cancelInPage(tabId, operationID) {
    try {
      const result = await this.runInPage(tabId, { kind: "cancel", operation_id: operationID });
  ...
  sessionPayload(value) {
    if (!isRecord(value) || !isRecord(value.fingerprint) || typeof value.logged_in !== "boolean") return null;
