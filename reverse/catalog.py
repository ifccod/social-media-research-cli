...
平台包是各自的限界上下文，其解析器就是公开接口契约。命令分发、JSON 发现、
Markdown 输出和生成的 Skill 都读取同一组声明。
"""
...
def _parameter(action: argparse.Action) -> Parameter:
    names = tuple(action.option_strings) or (action.dest,)
    choices = tuple(str(item) for item in action.choices or ())
    value_type = getattr(action.type, "__name__", "") if action.type else ""
    if not value_type:
        value_type = "flag" if action.nargs == 0 else "string"
    return Parameter(
        names=names,
...
def _format_usage(parser: argparse.ArgumentParser) -> str:
    ...
    return f"用法:{usage.removeprefix('usage:')}" if usage.startswith("usage:") else usage
...
def _commands(parser: argparse.ArgumentParser) -> tuple[Command, ...]:
    ...
    if subparsers is None:
        return ()
    summaries = {
        action.dest: "" if action.help in {None, argparse.SUPPRESS} else str(action.help)
        for action in subparsers._choices_actions
    }
    commands: list[Command] = []
    for name, command_parser in subparsers.choices.items():
        command_parser.prog = f"{parser.prog} {name}"
        level = str(
            command_parser.get_default("_command_level") or "primitive"
        )
        if level not in {"primitive", "workflow"}:
            raise ValueError(f"invalid command level: {level}")
        parameters = tuple(
            _parameter(action)
        ...
        commands.append(
            Command(
                ...
                usage=_format_usage(command_parser),
...
def _parser_parameters(parser: argparse.ArgumentParser) -> tuple[Parameter, ...]:
    return tuple(
        _parameter(action)
        for action in parser._actions
        if not isinstance(
...
def platform_catalog() -> tuple[Platform, ...]:
    result: list[Platform] = []
    for name, module_name in PLATFORM_MODULES.items():
        cli = importlib.import_module(f"{__package__}.{module_name}.cli")
        parser = cli._parser()
        parser.prog = f"python -m reverse {name}"
        result.append(
            Platform(
                name=name,
                module=module_name,
                ...
                parameters=_parser_parameters(parser),
...
def interface_document() -> dict[str, Any]:
    from .browser_session import _parser as session_parser
    ...
    return {
        ...
        "runtime": "python",
        "entrypoint": "reverse",
        "platforms": [asdict(platform) for platform in platforms],
        "services": [
            {
                "name": "session",
                "description": session.description or "",
                "usage": session.format_usage().strip(),
                "parameters": [
                    asdict(parameter) for parameter in _parser_parameters(session)
                ],
                "commands": [asdict(command) for command in _commands(session)],
            }
        ],
        "request_policy": list(RATE_POLICY),
    }


def select_document(platform: str = "", command: str = "") -> dict[str, Any]:
    document = interface_document()
    if not platform:
        return document
    if platform == "session":
        selected = document["services"][0]
        if command:
            commands = [item for item in selected["commands"] if item["name"] == command]
            if not commands:
                raise KeyError(f"session/{command}")
            selected = {**selected, "commands": commands}
        return {**document, "platforms": [], "services": [selected]}
    matches = [item for item in document["platforms"] if item["name"] == platform]
    if not matches:
    ...
    if command:
        commands = [item for item in selected["commands"] if item["name"] == command]
        if not commands:
            raise KeyError(f"{platform}/{command}")
        selected = {**selected, "commands": commands}
    return {**document, "platforms": [selected], "services": []}


def _parameter_description(parameter: dict[str, Any]) -> str:
    details = [parameter["help"]] if parameter["help"] else []
    if parameter["choices"]:
        choices = ", ".join(f"`{item}`" for item in parameter["choices"])
        details.append(f"可选值：{choices}")
    if parameter["default"] is not None:
        default = json.dumps(parameter["default"], ensure_ascii=False)
        details.append(f"默认值：`{default}`")
    return "<br>".join(details).replace("|", "\\|")
...
def _append_parameter_table(
    ...
    for parameter in parameters:
        names = ", ".join(f"`{name}`" for name in parameter["names"])
        lines.append(
            f"| {names} | `{parameter['value_type']}` | "
            f"{'是' if parameter['required'] else '否'} | "
            f"{_parameter_description(parameter)} |"
        )
    lines.append("")
...
def render_markdown(document: dict[str, Any]) -> str:
    lines = [
        "# reverse CLI 接口",
        "",
        "> 本文由 `argparse` 声明生成。修改解析器的 help 或 docstring 后请重新生成。",
        "",
        "```text",
        "python -m reverse <platform> <command> [options]",
        "python -m reverse session <command> [options]",
        "```",
        "",
    ]
    for section in [*document["platforms"], *document["services"]]:
        ...
        if section["parameters"]:
            ...
            _append_parameter_table(lines, section["parameters"])
        for command in section["commands"]:
            lines.extend(
                [
                    ...
                    "",
                    command["summary"] or command["description"],
                    "",
                    "```text",
                    command["usage"],
                    "```",
            ...
            _append_parameter_table(lines, command["parameters"])
    return "\n".join(lines).rstrip() + "\n"
...
def render_skill() -> str:
    ...
    command_count = sum(len(platform.commands) for platform in catalog)
    session_commands = _commands(session_parser())
    lines = [
        "---",
        "name: reverse",
        ...
        "",
        f"`reverse` 提供 {len(catalog)} 个平台上下文和 {command_count} 条命令。",
        ...
        "## 浏览器会话",
        "",
        ", ".join(f"`{command.name}`" for command in session_commands),
        "",
    ...
    lines.extend(f"- {rule}" for rule in RATE_POLICY)
    lines.extend(
        [
            "",
            "## 调用层级",
